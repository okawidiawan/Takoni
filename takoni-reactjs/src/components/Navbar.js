...
const Navbar = ({ isLogin }) => {
  return (
    <div className={`mt-10 flex items-center justify-between rounded-md bg-white px-8 py-4 shadow-md ${isLogin ? "hidden" : ""} `}>
      <div className="logo">
        <Link to="/">
      ...
      <div className="menu">
        <ul className="flex  w-96 justify-between">
          ...
          {!isLogin ? (
            ...
            <li>
              <Link to="/dashboard" className="block h-7 w-7 rounded-full bg-[#3E4154] text-center text-xl font-bold text-white shadow-md shadow-[#3E4154]/60 transition duration-300 hover:bg-[#3E4154]/90">
                O
              </Link>
            </li>
          )}
